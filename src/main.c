...
void wall_detected(enum SENSOR_DIRECTION sensor_direction)
{
    if (sensor_direction == LEFT_SENSOR && gpio_get(FRONT_IR_SENSOR_PIN) == 0)
    {
        car_state = ADJUSTING;
    }
    else if (sensor_direction == FRONT_SENSOR && !(car_state == TURNING))
    {
        car_state = TURNING;
    }
    else
...
void main_task(void *params)
{
    ...
    while (true)
    {
        ...
        case ADJUSTING:
            ...
            while (gpio_get(LEFT_IR_SENSOR_PIN) == 1)
                ;
            car_state = MOVING;
            break;
